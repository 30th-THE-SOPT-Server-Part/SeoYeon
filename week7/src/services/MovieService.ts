...
const getMoviesBySearch = async(search: string): Promise<MovieInfo[]> => { //가공처리 안하고 바로 반환할거라서 반환값을 Promise<MovieInfo[]>로 함
    const regex = (pattern: string) => new RegExp(`.*${pattern}.*`);

    try{
        const titleRegex: RegExp = regex(search); // search를 정규표현식으로 바꾼 것

        const movies = await Movie.find({ title: { $regex: titleRegex }}); //&regex 연산자 이용

        return movies; //찾은 것을 반환
    } catch(error) {
        console.log(error);
        throw error;
    }
